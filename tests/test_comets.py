...
def test_comets_excluded_from_normal_candidate_selection():
    ...
    assert [p.id for p in select_candidates(source, [source, comet, normal], 0, {1}).candidates] == [2]
...
def test_forced_comet_launch_generated_after_trigger():
    ...
    obs1 = {"player": 0, "comet_planet_ids": [9], "planets": [[9, -1, 50, 50, 1, 5, 1], [1, -1, 55, 50, 1, 5, 1]]}
    assert controller.update_and_forced_actions(obs1, 0) == []
    obs2 = {"player": 0, "comet_planet_ids": [9], "planets": [[9, 0, 50, 50, 1, 8, 1], [1, -1, 55, 50, 1, 5, 1]]}
